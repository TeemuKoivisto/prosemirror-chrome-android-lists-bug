...
import { Fragment, NodeType, Slice, Attrs } from '@tiptap/pm/model'
  ...
  return ({ tr, state, dispatch }) => {
    ...
    if ($from.parent.content.size == 0 && $from.node(-1).childCount == $from.indexAfter(-1)) {
      ...
      if (dispatch) {
        ...
        state.tr.replace(start, $from.after(-depthAfter), new Slice(wrap, 4 - depthBefore, 0))
        let sel = -1
    ...
    tr = state.tr.delete($from.pos, $to.pos)
    let types = nextType
